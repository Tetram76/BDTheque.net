...
public abstract class EditableEntityRepository<TEntity>(IServiceProvider serviceProvider, BDThequeContext dbContext)
    : EntityRepository<TEntity, Guid>(serviceProvider, dbContext), IEditableEntityRepository<TEntity>
    where TEntity : class, IUniqueIdEntity
{
    ...
    private async Task<ValidationResult?> GetValidationResult(TEntity entity, CancellationToken cancellationToken)
    {
        ...
        if (ServiceProvider.GetService(typeof(IValidator<TEntity>)) is IValidator<TEntity> validator)
        {
